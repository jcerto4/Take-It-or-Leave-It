...
public class PokerSolver {
	...
	/*
	 * Evaluate hand to determine rank: High Card: 1 Pair: 2 Two Pair: 3 Three of a
	 * Kind: 4 Straight: 5 Flush: 6 Full House: 7 Four of a Kind: 8 Straight Flush:
	 * 9 Five of a Kind: 10 (Deuces Wild) Wild Royal Flush: 11 (Deuces Wild) Four
	 * Deuces: 12 (Deuces Wild) Royal Flush: 13 Natural Royal Flush: 13 (Deuces
	 * Wild)
	 */
	...
	private static boolean checkGroups(Hand hand) {
		...
		if (deuces == 4) {
			...
			// 5 of a kind
			if (checkRank == 0 && (deuces == 1 && pairCounts[0] == 4) || (deuces == 2 && pairCounts[0] == 3)
			...
			if (checkRank == 0 && (deuces == 1 && pairCounts[0] == 3) || (deuces == 2 && pairCounts[0] == 2)) {
				checkRank = 8;
		...
		if (pairFace[0] > 0) {
			...
			if (pairFace[1] > 0) {
				suffix = (pairFace[0] > pairFace[1]) ? suffix + " & " + cardFaces[pairFace[1] - 1] + "s"
	...
	public static int[] evaluatePokerGame(Hand... hands) {
		...
		for (int i = 1; i < handRanks.length; i++) {
			if (handRanks[i] == handRanks[i - 1]) { // same rank, check score
				int handScoreCompare = handScores[i] > handScores[i - 1] ? 1
						: handScores[i] < handScores[i - 1] ? -1 : 0;
